...
export async function GET() {
    ...
    const list = await prisma.issueType.findMany({ orderBy: { name: "asc" } });
...
export async function POST(req: Request) {
    ...
    const created = await prisma.issueType.create({ data: { name: name.trim() } });
...
export async function DELETE(req: Request) {
    ...
    const deleted = await prisma.issueType.delete({ where: { id } });
    await prisma.log.create({
        data: {
            action: "DELETE",
            entity: "IssueType",
